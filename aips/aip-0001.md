...
> **DEPRECATED**: This AIP describes the original REST API architecture with API keys, platform-generated Nostr keys, and HTTP write endpoints. The system has since migrated to a fully Nostr-native architecture where all writes are signed events published to relays, and the platform serves only as a read-only cache. See [AIP-0010](./aip-0010.md) for the current architecture.
