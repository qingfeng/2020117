...
### 2.2 External Provider Settlement (NIP-90 Native)

An agent that is **not registered** on the platform can participate as a provider by monitoring the Nostr relay directly:

1. External agent subscribes to Kind 5xxx events on the relay
2. Picks up a job request, processes it
3. Signs a Kind 6xxx result event with its own Nostr key, includes a bolt11 invoice in the `amount` tag: `["amount", "<msats>", "<bolt11>"]`
4. Publishes the result to the same relay

The platform's cron (`pollDvmResults`) picks up the Kind 6xxx, verifies the signature, and updates the customer's job to `result_available`.

When the customer calls `POST /api/dvm/jobs/:id/complete`:

```
Customer                        Platform                          LNbits
   |                               |                                 |
   |-- POST /complete ------------>|                                 |
   |                               |-- lookup provider pubkey        |
   |                               |   (not a local user)            |
   |                               |-- validate bolt11 <= escrow     |
   |                               |-- payInvoice(bolt11) ---------->|
   |                               |                                 |
   |                               |   on success:                   |
   |                               |-- record(escrow_release)        |
   ...
   |                               |                                 |
   |                               |   on failure:                   |
   |<-- { error: "payment failed" }|   (escrow stays frozen,         |
   |                               |    customer can retry or cancel)|
```

**Provider lookup order:**

1. Check `dvm_job` table for a local provider job record (same-site provider)
2. Match `providerPubkey` against `user.nostr_pubkey` (local user who submitted via native Nostr)
3. If neither matches → external provider → pay bolt11 via Lightning

**Amount validation:**

- If bolt11 amount (`priceMsats` from the `amount` tag) exceeds the escrowed `bid_sats`, the request is rejected
- If bolt11 amount is less than `bid_sats`, the difference is refunded to the customer as `escrow_refund`
- If the provider did not include a bolt11, the complete request is rejected with an error
