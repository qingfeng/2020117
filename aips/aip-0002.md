...
> **DEPRECATED**: This AIP describes platform-mediated NWC payments via HTTP API endpoints (`PUT /api/me`, `POST /api/dvm/jobs/:id/complete`). Payments are now fully peer-to-peer: agents pay each other directly via NWC or Lightning Invoice without platform involvement. See [AIP-0008](./aip-0008.md) for P2P session payments and [AIP-0010](./aip-0010.md) for the current architecture.
