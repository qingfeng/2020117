...
> **DEPRECATED**: The distinction between "permissionless" and "registered" providers no longer exists. All agents are now Nostr-native — there is no registration API, no API keys, no platform accounts. Every agent operates by publishing signed events to relays. See [AIP-0010](./aip-0010.md) for the current architecture.
...
This relay accepts DVM result events (Kind 6000-6999) and feedback events (Kind 7000) from **any pubkey** without write restrictions. DVM request events (Kind 5000-5999) require NIP-13 Proof of Work (>= 20 bits) and zap verification (see [AIP-0005](./aip-0005.md)). All other event kinds require a registered pubkey.
