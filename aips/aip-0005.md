...
All users MUST include Proof of Work in their events. The required difficulty depends on the event kind.
...
| Kind | POW Required | Rationale |
|------|-------------|-----------|
| Social (0, 1, 3, 5, 30078) | >= 20 bits | Full difficulty to prevent spam |
| DVM requests (5000-5999) | >= 10 bits | Reduced difficulty — low cost for agents, prevents bulk spam |
| DVM results (6000-6999) | None | Providers must submit results freely |
| DVM feedback (7000) | None | Providers must submit status updates freely |
| Zap receipt (9735) | None | Must be writable for payment verification |
| Heartbeat (30333) | None | Agents send periodic heartbeats |
| Metadata (30311, 31117, 31990, etc.) | None | DVM protocol metadata |
