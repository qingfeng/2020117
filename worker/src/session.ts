    ...
    switch (msg.type) {
      ...
      case 'session_tick': {
        ...
        if (msg.bolt11) {
          ...
          if (!nwcParsed) {
            warn('No NWC wallet configured — ending session')
            endSession()
            break
          }
          try {
          ...
          warn('No invoice in session_tick — ending session')
          endSession()
        }
        break
      }
...
async function main() {
  ...
  if (nwcUri) {
    ...
    warn('No NWC wallet configured.')
    warn('  Use: --nwc="nostr+walletconnect://..." or set nwc_uri in .2020117_keys')
    await node.destroy()
    process.exit(1)
  }
